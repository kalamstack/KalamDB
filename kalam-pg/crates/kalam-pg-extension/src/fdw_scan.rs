...
unsafe fn begin_foreign_scan_impl(node: *mut pg_sys::ForeignScanState) -> Result<(), KalamPgError> {
    use kalam_pg_api::{ScanRequest, TenantContext};
    ...
    let tenant_context = TenantContext::new(None, user_id.clone());
    ...
    // Execute scan against embedded runtime
    ...
    for i in 0..natts {
        ...
        // Virtual columns don't map to Arrow
        if col_name == USER_ID_COLUMN || col_name == SEQ_COLUMN || col_name == DELETED_COLUMN {
...
unsafe fn begin_foreign_scan_impl(
    _node: *mut pg_sys::ForeignScanState,
) -> Result<(), KalamPgError> {
    Err(KalamPgError::Unsupported(
        "scan requires embedded or remote feature".to_string(),
    ))
}
