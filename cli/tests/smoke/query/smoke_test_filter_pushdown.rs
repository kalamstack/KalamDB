...
fn scalar_string(row: &HashMap<String, Value>, column: &str) -> String {
    let value = extract_typed_value(row.get(column).unwrap_or_else(|| {
        panic!("expected column '{}' in row {:?}", column, row)
    }));
    match value {
...
fn smoke_test_filter_pushdown() {
    if !is_server_running() {
        println!(
            "Skipping smoke_test_filter_pushdown: server not running at {}",
            server_url()
        );
        return;
    }
    ...
    for sql in [
        ...
        format!(
            "INSERT INTO {} (event_id, payload) VALUES ('evt-2', 'beta')",
            full_stream_table
        ),
        format!(
            "INSERT INTO {} (event_id, payload) VALUES ('evt-3', 'beta')",
            full_stream_table
        ),
    ] {
    ...
    let system_rows = query_rows(&format!(
    ...
    let shared_pk_rows = query_rows(&format!(
    ...
    let shared_name_ids: Vec<String> = shared_name_rows
        .iter()
        .map(|row| scalar_string(row, "id"))
        .collect();
    assert_eq!(shared_name_ids, vec!["2".to_string(), "3".to_string()]);

    let stream_rows = query_rows(&format!(
        "SELECT event_id FROM {} WHERE event_id = 'evt-2'",
        full_stream_table
    ));
