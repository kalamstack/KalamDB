...
use kalamdb_jobs::{AppContextJobsExt, FlushScheduler};
use tokio::time::Duration;

use super::test_support::{
    auth_helper::create_user_auth_header_with_id,
...
async fn count_matching_flush_jobs(
    ...
    let resp = server
        .execute_sql(
            "SELECT parameters FROM system.jobs WHERE job_type = 'flush' ORDER BY created_at DESC \
             LIMIT 500",
        )
        .await?;
    anyhow::ensure!(
...
async fn test_flush_policy_and_parquet_output_over_http() {
    (async {
        ...
        // -----------------------------------------------------------------
        {
            ...
            for i in 0..25 {
                let resp = server
                    .execute_sql_with_auth(
                        &format!(
                            "INSERT INTO {}.{} (id, content) VALUES ({}, 'msg-{}')",
                            ns, table, i, i
                        ),
                        &auth_a,
                    )
            ...
            for batch in 0..3 {
                for i in 0..10 {
                    ...
                    let resp = server
                        .execute_sql_with_auth(
                            &format!(
                                "INSERT INTO {}.{} (id, payload) VALUES ({}, 'p-{}')",
                                ns, table, id, id
                            ),
                            &auth_a,
                        )
        ...
        // -----------------------------------------------------------------
        {
            ...
            for i in 0..15 {
                let resp = server
                    .execute_sql_with_auth(
                        &format!(
                            "INSERT INTO {}.{} (id, body) VALUES ({}, '{}-msg-{}')",
                            ns, table, i, user_a, i
                        ),
                        &auth_a,
                    )
        ...
        // -----------------------------------------------------------------
        {
            ...
            let resp = server
                .execute_sql(&format!(
                    "CREATE TABLE {}.{} (id BIGINT PRIMARY KEY, entry TEXT) WITH (TYPE='SHARED', \
            ...
            flush_table_and_wait(server, &ns, table).await?;
            let _ =
                wait_for_parquet_files_for_table(server, &ns, table, 1, Duration::from_secs(40))
                    .await?;
        }
...
async fn test_automatic_flush_waits_for_row_limit_before_writing_parquet_over_http() {
    (async {
        ...
        let resp = server
            .execute_sql(&format!("INSERT INTO {}.{} (id, payload) VALUES (4, 'row-4')", ns, table))
            .await?;
        anyhow::ensure!(
            resp.status == ResponseStatus::Success,
            "threshold INSERT failed: {:?}",
            resp.error
        );
        ...
        loop {
            ...
            if tokio::time::Instant::now() >= flush_deadline {
                anyhow::bail!(
                    "expected parquet files to be written after the threshold flush job executed"
                );
            }
...
async fn test_automatic_user_flush_waits_for_row_limit_and_writes_only_user_files_over_http() {
    (async {
        ...
        let resp = server
            .execute_sql_with_auth(
                &format!("INSERT INTO {}.{} (id, payload) VALUES (4, 'user-a-4')", ns, table),
                &auth_a,
            )
            .await?;
        anyhow::ensure!(
        ...
        loop {
            ...
            if tokio::time::Instant::now() >= flush_deadline {
                anyhow::bail!(
                    "expected user-table parquet files after the threshold flush job executed"
                );
            }
        ...
        let files_after_first_flush =
            count_parquet_files_for_user(&storage_root, &ns, table, &user_a_id);
