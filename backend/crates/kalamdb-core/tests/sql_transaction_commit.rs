...
#[tokio::test]
#[ntest::timeout(15000)]
async fn sql_request_transaction_commit_persists_rows() {
    let (app_ctx, executor, table_id, _test_db) =
        setup_shared_table("sql_tx_commit", "items").await;
    let request_ctx = request_exec_ctx(&app_ctx, "sql-request-commit");
