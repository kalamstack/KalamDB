...
use crate::conversions::{json_value_to_scalar_for_column, scalar_to_json_for_column};
use crate::models::rows::Row;
use crate::schemas::TableDefinition;
...
pub fn serde_model_to_row<T: Serialize>(model: &T, table_def: &TableDefinition) -> Result<Row, String> {
    let value = serde_json::to_value(model)
        .map_err(|error| format!("model serialize failed: {error}"))?;
    let object = value
        .as_object()
...
pub fn row_to_serde_model<T: DeserializeOwned>(row: &Row, table_def: &TableDefinition) -> Result<T, String> {
