...
async fn test_disconnect_after_ack_before_first_initial_batch() {
    let result = timeout(Duration::from_secs(60), async {
        ...
        for _ in 0..40 {
            ...
            if disconnect_count.load(Ordering::SeqCst) >= expected_disconnects {
                break;
            }
        }

        wait_for_disconnect_count(&disconnect_count, expected_disconnects, "ack-gap outage").await;
        ...
        proxy.resume();
        wait_for_reconnect(&client, &connect_count, expected_connects, "ack-gap recovery").await;
