...
use crate::{
    ...
    models::{ConnectionOptions, HttpVersion, QueryResponse, SubscriptionConfig},
...
impl KalamLinkClient {
    ...
    pub async fn cluster_health_check(&self) -> Result<ClusterHealthResponse> {
        ...
        if !status.is_success() {
            ...
            let message = if status.as_u16() == 403 {
                format!(
                    "Cluster health endpoint is restricted to localhost connections ({})",
                    body
                )
            } else {
                format!("HTTP {} — {}", status, body)
    ...
    /// ```rust,no_run
    /// # async fn example() -> Result<(), Box<dyn std::error::Error>> {
    /// use kalam_client::{KalamLinkClient, AuthProvider};
    ///
    ...
    ///
    /// ```rust,no_run
    /// use kalam_client::{KalamLinkClient, ServerSetupRequest};
    ///
    /// # async fn example() -> kalam_client::Result<()> {
    /// let client = KalamLinkClient::builder()
    ///     .base_url("http://localhost:8080")
...
impl KalamLinkClientBuilder {
    ...
    ///
    /// ```rust,no_run
    /// use kalam_client::{AuthProvider, DynamicAuthProvider, KalamLinkClient, Result};
    /// use std::sync::Arc;
    /// use std::pin::Pin;
    ...
    ///
    /// ```rust,no_run
    /// use kalam_client::{KalamLinkClient, KalamLinkTimeouts};
    ///
    /// # async fn example() -> kalam_client::Result<()> {
    /// let client = KalamLinkClient::builder()
    ///     .base_url("http://localhost:3000")
    ...
    ///
    /// ```rust,no_run
    /// use kalam_client::{KalamLinkClient, ConnectionOptions, HttpVersion};
    ///
    /// # async fn example() -> kalam_client::Result<()> {
    /// let client = KalamLinkClient::builder()
    ///     .base_url("http://localhost:3000")
    ...
    ///
    /// ```rust,no_run
    /// use kalam_client::{KalamLinkClient, HttpVersion};
    ///
    /// # async fn example() -> kalam_client::Result<()> {
    /// let client = KalamLinkClient::builder()
    ///     .base_url("http://localhost:3000")
