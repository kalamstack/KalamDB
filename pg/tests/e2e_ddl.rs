...
// =========================================================================
// Test 16: CREATE TABLE ... USING kalamdb with KalamDB-native syntax
// =========================================================================
...
async fn e2e_ddl_create_table_using_kalamdb_basic() {
    ...
    // CREATE TABLE ... USING kalamdb with KalamDB-native syntax
    pg.batch_execute(&format!(
    ...
    // Verify: table exists in KalamDB
    assert!(
        env.kalamdb_table_exists(&ns, &table).await,
        "KalamDB table {ns}.{table} should exist after CREATE TABLE USING kalamdb"
    );

    // Verify: PG has the foreign table with correct columns
    let pg_rows = pg
        .query(
    ...
    let pg_cols: Vec<String> = pg_rows.iter().map(|r| r.get(0)).collect();
    ...
    // Verify: KalamDB has the columns with proper types
    ...
    // Verify: KalamDB has PRIMARY KEY and SNOWFLAKE_ID default on id column
    let columns = env
        .kalamdb_sql(&format!(
    ...
    assert!(
        id_row
            ...
            .map(|c| {
                let default_val = c.get(1).and_then(|v| v.as_str()).unwrap_or("");
                let is_pk = c.get(2).and_then(|v| v.as_bool()).unwrap_or(false);
...
// =========================================================================
// Test 17: CREATE TABLE USING kalamdb — multiple defaults forwarded
// =========================================================================
...
async fn e2e_ddl_create_table_using_kalamdb_multiple_defaults() {
    ...
    assert!(
        env.kalamdb_table_exists(&ns, &table).await,
        "KalamDB table should exist"
    );

    // Verify defaults are stored in KalamDB
    let columns = env
        .kalamdb_sql(&format!(
    ...
    // id → SNOWFLAKE_ID
    let id_default = rows.iter().find(|row| {
        row.as_array().and_then(|c| c.first()).and_then(|v| v.as_str()) == Some("id")
    }).and_then(|r| r.as_array()).and_then(|c| c.get(1)).and_then(|v| v.as_str()).unwrap_or("");
    assert!(id_default.contains("SNOWFLAKE_ID"), "id should have SNOWFLAKE_ID default, got: {}", id_default);

    // label → 'untitled'
    let label_default = rows.iter().find(|row| {
        row.as_array().and_then(|c| c.first()).and_then(|v| v.as_str()) == Some("label")
    }).and_then(|r| r.as_array()).and_then(|c| c.get(1)).and_then(|v| v.as_str()).unwrap_or("");
    assert!(label_default.contains("untitled"), "label should have 'untitled' default, got: {}", label_default);
...
// =========================================================================
// Test 18: CREATE TABLE USING kalamdb — WITH options forwarded to KalamDB
// =========================================================================
...
async fn e2e_ddl_create_table_using_kalamdb_with_options() {
    ...
    // Verify table was created as USER type in KalamDB
    let table_info = env
        .kalamdb_sql(&format!(
...
// =========================================================================
// Test 19: CREATE TABLE USING kalamdb — DML roundtrip (INSERT + SELECT)
// =========================================================================
...
async fn e2e_ddl_create_table_using_kalamdb_dml_roundtrip() {
    ...
    // Verify id was auto-generated (not null, not 0)
    let id_rows = pg
        .query(&format!("SELECT id FROM {ns}.{table}"), &[])
        .await
        .expect("SELECT id");
    for row in &id_rows {
